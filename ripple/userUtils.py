...
def updateStats(userID, __score, beatmap_=None):
	"""
	...
	if beatmap_ is None:
		beatmap_ = objects.beatmap.beatmap(__score.fileMd5, 0)
	...
	if (__score.mods & mods.DOUBLETIME) > 0:
		realMapLength //= 1.5
	elif (__score.mods & mods.HALFTIME) > 0:
		realMapLength //= 0.75
	glob.db.execute(
		...
		(__score.score, realMapLength, userID)
	)
...
def updateRanks(userID=0, score=None):
	...
	if score is not None:
		userID = score.playerUserID
		rank = generalUtils.getRank(int(score.gameMode), int(score.mods), int(score.accuracy) * 100, int(score.c300), int(score.c100), int(score.c50), int(score.cMiss))
	if score.passed and rank == 'XH':
		glob.db.execute("UPDATE users_stats SET XH = XH + 1 WHERE id = {} LIMIT 1".format(userID))
	if score.passed and rank == 'X':
		glob.db.execute("UPDATE users_stats SET X = X + 1 WHERE id = {} LIMIT 1".format(userID))
	if score.passed and rank == 'SH':
		glob.db.execute("UPDATE users_stats SET SH = SH + 1 WHERE id = {} LIMIT 1".format(userID))
	if score.passed and rank == 'S':
		glob.db.execute("UPDATE users_stats SET S = S + 1 WHERE id = {} LIMIT 1".format(userID))
	if score.passed and rank == 'A':
		glob.db.execute("UPDATE users_stats SET A = A + 1 WHERE id = {} LIMIT 1".format(userID))